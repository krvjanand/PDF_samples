...
def initialize_vertex_ai(key_file, project_id, location, model_id):
    try:
        ...
        system_instructions = (
            ...
            "Output formatting: Extract only the following invoice data from the PDF document in a valid JSON format only. Avoid unnecessary quotes, comma, text like json’’’, ‘’’json, etc. in the generated structured output"
        )
...
def check_for_duplicates(json_response):
    master_df = pd.read_excel('C:/Users/dass__000/PycharmProjects/pythonProject1/venv/Master_Invoice_Data.xlsx')

    #Compare the dataframe with master DF
    print(master_df['Invoice Number'])
    print(master_df['Invoice Number'].tolist())
    print(type(master_df['Invoice Number'].tolist()[0]))
    print(json_response['Invoice Number #'])
    print(type(master_df['Invoice Number #']))

    if str(json_response['Invoice Number #']) in [str(x) for x in master_df['Invoice Number'].tolist()]:
        return "Duplicate"
    else:
        return "Not Duplicate"


def save_duplicate_invoice(uploaded_file, file_content, folder_path):
...
def generate_content_and_confidence(gcs_uri, prompt, model):
    json_response = generate_content(gcs_uri, prompt, model)
    if json_response:
        json_string = json.dumps(json_response)
        gen_prompt = json_string + "Generate Confidence Score in percentage for the PDF document and prompt and return only the 'Confidence Score: percentage value' as output. No other unwanted text or explanation in the output. Example: Confidence Score: 93%"
        confidence_score = generate_confidence(gcs_uri, gen_prompt, model)
    return json_response, confidence_score
...
def main():
    ...
    with st.sidebar:
        st.title("Streamline Invoice Processing")
    ...
    with col1:
        ...
        st.title("Automate Hospital AP Invoices & Receipts Processing")
        st.image(ind_usa_flag_image, width=60, use_column_width=False)

    with col2:
        col2.image('C:/Users/dass__000/PycharmProjects/pythonProject1/venv/Invoice Processing_Duplication Check_resized.gif')
    ...
    if choice == 'Value Proposition':
        ...
        # st.title("EazyEnroll")
        st.subheader("Effortless Invoice & Receipts Processing: Data extraction and Duplicate detection")

        uploaded_file = st.file_uploader("Upload Invoice file", type="pdf")
        ...
        if uploaded_file is not None:
            ...
            json_response, confidence_score = generate_content_and_confidence(gcs_uri, prompt, model)
            ...
            with col2:
                ...
                check_duplicate_button = st.button("Check Invoice Duplication")
                if check_duplicate_button:
                    if json_response is not None:
                        if check_for_duplicates == "Duplicate":
                            save_duplicate_invoice(uploaded_file, file_content, "C:/Users/dass__000/PycharmProjects/pythonProject1/venv/Save_Duplicate_Invoices")
                            st.error("Result: Duplicate Invoice")
                    else:
                        st.success("Result: Not a Duplicate Invoice")
                else:
                    st.error("Please upload an Invoice.")

        
    elif choice == "About":
        st.title("About Invoice Check")
